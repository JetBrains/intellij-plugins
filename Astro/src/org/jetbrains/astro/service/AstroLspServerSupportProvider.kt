...
class AstroLspServerSupportProvider : LspServerSupportProvider {
  override fun fileOpened(project: Project, file: VirtualFile, serverStarter: LspServerStarter) {
    getAstroServerDescriptor(project, file)?.let { serverStarter.ensureServerStarted(it) }
  }
}

fun getAstroServerDescriptor(project: Project, file: VirtualFile): LspServerDescriptor? {
  if (!isServiceEnabledAndAvailable(project, file)) return null
  return AstroLspServerDescriptor(project)
}

class AstroLspServerDescriptor(project: Project)
  : JSFrameworkLspServerDescriptor(project, "Astro") {
