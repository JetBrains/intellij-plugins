 ...
 */
public class PrettierOnSaveActivationTest extends BasePlatformTestCase {

  /**
   * {@link com.intellij.testFramework.LightPlatformTestCase} reuses its light project unless the descriptor differs by
   * identity, so a dedicated instance is needed here: sharing the project with a test that already asked for the
  ...
  public void testSaveDoesNotCreateConfig() {
    assertNull("precondition: the Prettier configuration was already created during test setup",
               getProject().getServiceIfCreated(PrettierConfiguration.class));
    ...
    assertNull("the save path must not create the Prettier configuration service (WEB-78877)",
               getProject().getServiceIfCreated(PrettierConfiguration.class));
  }
}
