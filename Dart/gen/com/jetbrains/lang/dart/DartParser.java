...
public class DartParser implements PsiParser, LightPsiParser {
  ...
  /* ********************************************************** */
  // metadata* 'enum' componentName typeParameters? mixins? interfaces? '{' enumConstantDeclaration? (',' enumConstantDeclaration)* ','? ';'? classMemberDefinition* '}'
  ...
  private static boolean enumDefinition_11(PsiBuilder b, int l) {
    ...
    while (true) {
      int c = current_position_(b);
      if (!classMemberDefinition(b, l + 1)) break;
      if (!empty_element_parsed_guard_(b, "enumDefinition_11", c)) break;
    }
    return true;
  }
