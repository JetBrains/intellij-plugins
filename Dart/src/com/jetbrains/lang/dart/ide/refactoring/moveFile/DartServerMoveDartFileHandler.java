...
public class DartServerMoveDartFileHandler extends MoveFileHandler {
  ...
  public boolean canProcessElement(PsiFile psiFile) {
    ...
    return dasService.isInIncludedRoots(psiFile.getVirtualFile());
  }
  ...
  public void prepareMovedFile(PsiFile psiFile, PsiDirectory moveDestination, Map<PsiElement, PsiElement> oldToNewMap) {
    ...
    final String oldFilePath = FileUtil.toSystemDependentName(virtualFile.getPath());
    final String newFilePath = FileUtil.toSystemDependentName(moveDestination.getVirtualFile().getPath()) + "/" + virtualFile.getName();

    // Create the refactoring.
    final MoveFileRefactoring refactoring = new MoveFileRefactoring(project, virtualFile, oldFilePath, newFilePath);
