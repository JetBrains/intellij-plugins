...
public class DartProblemsViewPanel2 extends JPanel implements Disposable {
  final DartProblemsTableModel myModel = new DartProblemsTableModel();
    ...
    table.addMouseListener(new MouseAdapter() {
      ...
      public void mouseClicked(MouseEvent e) {
        ...
        if (e.getClickCount() == 2) {
          final AnalysisError analysisError = table.getSelectedObject();
          if (analysisError != null) {
            final Location location = analysisError.getLocation();
            final String filePath = location.getFile();
            final VirtualFile file = LocalFileSystem.getInstance().findFileByPath(filePath);
            if (file != null) {
              AssistUtils.navigate(project, file, location.getOffset());
            }
          }
        }
      }
  ...
  public DartProblemsTableModel() {
    super(new ColumnInfo<AnalysisError, AnalysisError>("Description") {
      ...
      public String valueOf(AnalysisError o) {
        ...
        return fileName + " [line " + location.getStartLine() + "]";
      }
