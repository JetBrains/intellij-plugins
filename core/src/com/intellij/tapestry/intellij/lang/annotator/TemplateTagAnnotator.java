...
public class TemplateTagAnnotator extends XmlRecursiveElementVisitor implements Annotator {
    ...
    public void visitXmlTag(XmlTag tag) {
        ...
        if (ComponentUtils.isComponentTag(new IntellijXmlTag(tag))) {
            ...
            _annotationHolder.createInfoAnnotation(tag.getChildren()[1], null).setTextAttributes(TextAttributesKey.find(TemplateColorSettingsPage.TAPESTRY_COMPONENT_TAG_KEY));

            // only annotation closing tag if the tag isn't empty like <t:body/>
            if (!tag.isEmpty()) {
                _annotationHolder.createInfoAnnotation(tag.getChildren()[tag.getChildren().length - 2], null).setTextAttributes(TextAttributesKey.find(TemplateColorSettingsPage.TAPESTRY_COMPONENT_TAG_KEY));
            }

            Component component;
            try {
                Module module = ProjectRootManager.getInstance(tag.getManager().getProject()).getFileIndex().getModuleForFile(tag.getContainingFile().getVirtualFile());
            ...
            if (component != null) {
                ...
                try {
                    elementClass = (IntellijJavaClassType) ComponentUtils.findClassFromTemplate(new IntellijResource(tag.getContainingFile()), tapestryProject);
                } catch (NotFoundException ex) {
                    elementClass = null;
                }
                ...
                if (elementClass != null) {
                    ...
                    for (TapestryParameter parameter : component.getParameters().values()) {
                        ...
                        _annotationHolder.createInfoAnnotation(attribute.getChildren()[0], null).setTextAttributes(TextAttributesKey.find(TemplateColorSettingsPage.TAPESTRY_COMPONENT_PARAMETER_KEY));
                        ...
                        if (!TypeCoercionValidator.canCoerce(tapestryProject, resolvedValue.getType(), AbstractValueResolver.getCleanValue(attribute.getValue()), parameterType)) {
                            _annotationHolder.createErrorAnnotation(attribute.getValueElement(),
                                    "Can't coerce a " + resolvedValue.getType().getName() + " to a " + (parameterType != null ? parameterType.getName() : "undefined"));
                        }
                    }
                }
            } else {
