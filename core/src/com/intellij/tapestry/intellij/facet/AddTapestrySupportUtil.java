...
public class AddTapestrySupportUtil {
    ...
    public static void addSupportInWriteCommandAction(final ModifiableRootModel rootModel, final WebFacet webFacet, final TapestryFacetConfiguration configuration, final boolean generateStartupApplication, final boolean generatePom) {
        new WriteCommandAction.Simple(webFacet.getModule().getProject(), getTapestryFiles(webFacet)) {
            protected void run() throws Throwable {
                try {
                    addSupport(rootModel, webFacet, configuration, generateStartupApplication, generatePom);
                }
                catch (Exception ex) {
                    _logger.error(ex);
                }
    ...
    private static void generatePom(ModifiableRootModel rootModel, TapestryFacetConfiguration configuration) throws IOException {
        ...
        try {
            String pomText = StringUtils.toString(AddTapestrySupportUtil.class.getResourceAsStream("/fileTemplates/j2ee/" + TapestryConstants.POM_TEMPLATE_NAME + ".ft"));
            ...
            PsiFile pomFile = PsiManager.getInstance(rootModel.getModule().getProject()).findDirectory(VirtualFileManager.getInstance().findFileByUrl("file://" + modulePath)).createFile("pom.xml");
    ...
    private static void generateStartupApplication(ModifiableRootModel rootModel, Module module, WebFacet webFacet, TapestryFacetConfiguration configuration) {
        ...
        if (pagesDirectory != null)
            try {
                String classText = StringUtils.toString(AddTapestrySupportUtil.class.getResourceAsStream("/fileTemplates/j2ee/" + TapestryConstants.START_PAGE_CLASS_TEMPLATE_NAME + ".ft"));
        ...
        if (servicesDirectory != null)
            try {
                String text = StringUtils.toString(AddTapestrySupportUtil.class.getResourceAsStream("/fileTemplates/j2ee/" + TapestryConstants.MODULE_BUILDER_CLASS_TEMPLATE_NAME + ".ft"));
                ...
                VirtualFile moduleBuilder = servicesDirectory.getVirtualFile().createChildData(servicesDirectory, StringUtil.capitalize(configuration.getFilterName()) + TapestryConstants.MODULE_BUILDER_SUFIX + ".java");
                VfsUtil.saveText(moduleBuilder, text);

                CodeStyleManager.getInstance(module.getProject()).reformat(servicesDirectory.findFile(StringUtil.capitalize(configuration.getFilterName()) + TapestryConstants.MODULE_BUILDER_SUFIX + ".java"));
            } catch (Exception ex) {
                _logger.error(ex);
            }
    }

    private static PsiFile[] getTapestryFiles(@NotNull final WebFacet webFacet) {
