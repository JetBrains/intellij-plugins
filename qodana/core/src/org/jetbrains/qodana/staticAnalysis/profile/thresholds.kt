...
class StateWithThreshold(context: QodanaGlobalInspectionContext, inspectionGroup: NamedInspectionGroup) : GroupState(context, inspectionGroup) {
  ...
  fun getProblemCounters(inspectionId: String): ProblemCounters? = thresholds.problemCounters[inspectionId]

}

/** Tracks the number of found problems, to skip further problems if one of the thresholds (file, module, project) is reached. */
internal class Thresholds(val project: Project, config: QodanaConfig) {
  val problemCounters: Map<String, ProblemCounters> = QodanaToolRegistrar.getInstance(project).createTools().associate {
...
class ProblemCounters(
  ...
  val perProject: AtomicInteger = AtomicInteger(0)
