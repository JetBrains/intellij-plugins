...
class TfConfigCompletionContributor : HilCompletionContributor() {
  init {
    ...
    HclRequiredProvidersCompletion.registerTo(this, TfPsiPatterns.TfRequiredProvidersBlock)

    // Block type or name
    HclBlockTypeNameCompletionProvider.registerTo(this, TerraformConfigFile)
  ...
  private object PropertyValueCompletionProvider : TfCompletionProvider() {
    override fun addCompletions(parameters: CompletionParameters, context: ProcessingContext, result: CompletionResultSet) {
      ...
      if (property.name == HCL_PROVIDER_IDENTIFIER && (type == HCL_RESOURCE_IDENTIFIER || type == HCL_DATASOURCE_IDENTIFIER)) {
        val providers = property.getTerraformModule().getProviderReferenceNames()
