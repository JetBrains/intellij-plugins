...
@Component({
  ...
  template: `
    ...
    <ng-container *ngrxLet="number$OrNot as number">{{number.toFixed()}}</ng-container>
    <ng-container *ngrxLet="numberOrNot$ as number">{{number.<error descr="Unresolved function or method toFixed()">toFixed</error>()}}</ng-container>
    ...
    <ng-container *ngrxLet="number$ as n1; let n2; error as e; complete as c">
      ...
      {{acceptBoolean(e)}}
      {{acceptBoolean(c)}}
      {{acceptBoolean(<error descr="Argument type LetViewContextValue<Observable<number>> is not assignable to parameter type boolean  Type number is not assignable to type boolean">n1</error>)}}
    </ng-container>
