...
public class DroolsLexer extends LookAheadLexer {
  ...
  protected void lookAhead(final @NotNull Lexer baseLexer) {
    ...
    else if (tokenType == IMPORT) {
      advanceLexer(baseLexer);
      addAllTokens(baseLexer, TokenSet.WHITE_SPACE);
      IElementType next =
        baseLexer.getTokenType(); // advance lexer for "import function" . otherwise processBlockExpressions(..) will be called
      if (next == FUNCTION) {
        advanceLexer(baseLexer);
      }
    }
