...
public abstract class Flex4Handler extends SdkSpecificHandler {
  ...
  static void setupConfiguration(final Builder builder, final Configuration configuration) {
    ...
    oemConfig.setConfiguration(new String[]{"-omit-trace-statements=" + configuration.getCompilerConfiguration().omitTraceStatements()});
    oemConfig.setConfiguration(new String[]{"-load-config="}); // "-load-config=" MUST BE THE LAST option passed via oemConfig.setConfiguration()
