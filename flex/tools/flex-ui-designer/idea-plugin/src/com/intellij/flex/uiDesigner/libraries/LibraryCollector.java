...
class LibraryCollector {
  ...
  private VirtualFile getRealFileIfValidSwc(final VirtualFile jarFile) {
    if (jarFile.getFileSystem() instanceof JarFileSystem) {
      ...
      if (file != null && !file.isDirectory() && file.getName().endsWith(".swc") && !isGlobalLibrary(file.getName()) &&
  ...
  private boolean isFlexSdkLibrary(VirtualFile file) {
    final String name = file.getName();
    if (name.startsWith(FlexLibsNames.FRAMEWORK)) {
      return JSResolveUtil.findClassByQName("FrameworkClasses", GlobalSearchScope.fileScope(project, file)) != null;
    }
    else if (name.startsWith(FlexLibsNames.SPARK)) {
      return JSResolveUtil.findClassByQName("SparkClasses", GlobalSearchScope.fileScope(project, file)) != null;
    }
  ...
  private void collectFromLibraryOrderEnrty(LibraryOrderEntry libraryOrderEntry) {
    ...
    for (VirtualFile jarFile : libraryOrderEntry.getRootFiles(OrderRootType.CLASSES)) {
      ...
      if (file != null && (!isAutogeneratedLibrary || !isAutomationOrUselessLibrary(file.getName()))) {
        addLibrary(file, jarFile, isFlexSdkLibrary(file));
      }
    }
  }
  ...
  private void addLibrary(VirtualFile file, VirtualFile jarFile, boolean isFromFlexSdk) {
    ...
    list.add(libraryManager.createOriginalLibrary(file, jarFile, initializer));
  }

  // IDEA-74117
  private static boolean isSwfAndCatalogExists(VirtualFile file) {
    if (Library.getSwfFile(file) == null || Library.getCatalogFile(file) == null) {
      LogMessageUtil.LOG.warn("SWC is corrupted (library.swf or catalog.xml doesn't exists): " + file.getPath());
      return false;
    }
