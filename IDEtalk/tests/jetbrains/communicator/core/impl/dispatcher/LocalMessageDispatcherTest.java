...
public class LocalMessageDispatcherTest extends BaseTestCase {
  ...
  public void testHistoryPersistence() throws Exception {
    ...
    MockMessage message = new MockMessage(new Date(), "some text по русски");
    myDispatcher.sendNow(myUser, message);
    ...
    assertEquals("some text по русски", ((MockMessage) localMessageDispatcher.getHistory(myUser, null)[0]).getMessage());
  }
