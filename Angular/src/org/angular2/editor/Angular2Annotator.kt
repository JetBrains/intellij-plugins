  ...
  override fun annotate(element: PsiElement, holder: AnnotationHolder) {
    when (element) {
      is JSProperty -> {
        ...
        val attributeInfo = Angular2AttributeNameParser.parse(name)
        val color = when (attributeInfo.type) {
      ...
      is JSLiteralExpression -> {
        val color = when {
          ...
          isHostBindingClassValueLiteral(element) -> CssHighlighter.CSS_CLASS_NAME
          else -> return
        }
