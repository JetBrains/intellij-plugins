...
var AngularVirtualCode = class {
  ...
  sourceFileUpdated(ts, snapshot, _languageId) {
    ...
    if (this.transpiledTemplate) {
      ...
      if (!getNormalizedSnapshotText(this.transpiledTemplate.snapshot).startsWith(sourceText) || sourceText !== this.transpiledTemplate.sourceCode[ts.server.toNormalizedPath(this.fileName)]) {
        templateInSync = false;
      } else if (this.transpiledTemplate.mappings.find((mapping) => {
        var _a, _b, _c;
        return getNormalizedSnapshotText((_a = this.ctx.getAssociatedScript(mapping.fileName)) == null ? void 0 : _a.snapshot) !== ((_c = (_b = this.transpiledTemplate) == null ? void 0 : _b.sourceCode) == null ? void 0 : _c[ts.server.toNormalizedPath(mapping.fileName)]);
      })) {
        templateInSync = false;
      }
    }
  ...
  transpiledTemplateUpdated(ts, transpiledCode, sourceCode, mappings) {
    if (transpiledCode) {
...
function decorateNgLanguageServiceExtensions(language, languageService, unboundReverseMapper, webStormGetElementType) {
  languageService.webStormNgUpdateTranspiledTemplate = (ts, fileName, transpiledCode, sourceCode, mappings) => {
    var _a;
