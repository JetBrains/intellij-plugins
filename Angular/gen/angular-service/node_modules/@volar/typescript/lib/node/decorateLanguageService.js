...
function decorateLanguageService(language, languageService) {
    ...
    languageService.getFormattingEditsForDocument = (filePath, options) => {
        ...
        if (serviceScript) {
            ...
            return edits
                .map(edit => takeIfSameName(fileName, (0, transform_1.transformTextChange)(sourceScript, map, edit, language_core_1.isFormattingEnabled)))
        ...
        if (serviceScript) {
            ...
            if (generatePosition !== undefined) {
                ...
                if (info) {
                    return {
                        ranges: info.ranges
                            .map(span => takeIfSameName(fileName, (0, transform_1.transformTextSpan)(sourceScript, map, span, language_core_1.isLinkedEditingEnabled)))
                            .filter(utils_1.notEmpty),
    ...
    languageService.provideCallHierarchyOutgoingCalls = (filePath, position) => {
        ...
        return calls
            ...
            const fromSpans = call.fromSpans
                ...
                ? takeIfSameName(fileName, (0, transform_1.transformTextSpan)(sourceScript, map, span, language_core_1.isCallHierarchyEnabled))
                : span)
                .filter(utils_1.notEmpty);
            return {
                to,
    ...
    languageService.getQuickInfoAtPosition = (filePath, position) => {
        ...
        if (serviceScript) {
            ...
            for (const [generatePosition, mapping] of (0, transform_1.toGeneratedOffsets)(sourceScript, map, position)) {
                ...
                if (info) {
                    const textSpan = takeIfSameName(fileName, (0, transform_1.transformTextSpan)(sourceScript, map, info.textSpan, language_core_1.isHoverEnabled));
                    if (textSpan) {
                        infos.push({
                            ...info,
                            textSpan: textSpan,
    ...
    languageService.getSignatureHelpItems = (filePath, position, options) => {
        ...
        if (serviceScript) {
            ...
            if (generatePosition !== undefined) {
                ...
                if (result) {
                    const applicableSpan = takeIfSameName(fileName, (0, transform_1.transformTextSpan)(sourceScript, map, result.applicableSpan, language_core_1.isSignatureHelpEnabled));
                    if (applicableSpan) {
                        return {
                            ...result,
    ...
    languageService.getRenameInfo = (filePath, position, options) => {
        ...
        if (serviceScript) {
            ...
            for (const [generateOffset, mapping] of (0, transform_1.toGeneratedOffsets)(sourceScript, map, position)) {
                ...
                if (info.canRename) {
                    const span = takeIfSameName(fileName, (0, transform_1.transformTextSpan)(sourceScript, map, info.triggerSpan, language_core_1.isRenameEnabled));
                    if (span) {
    ...
    languageService.getEncodedSemanticClassifications = (filePath, span, format) => {
        ...
        if (serviceScript) {
            ...
            for (let i = 0; i < result.spans.length; i += 3) {
                const sourceStart = takeIfSameName(fileName, (0, transform_1.toSourceOffset)(sourceScript, map, result.spans[i], language_core_1.isSemanticTokensEnabled));
                const sourceEnd = takeIfSameName(fileName, (0, transform_1.toSourceOffset)(sourceScript, map, result.spans[i] + result.spans[i + 1], language_core_1.isSemanticTokensEnabled));
    ...
    languageService.getSyntacticDiagnostics = filePath => {
        ...
        const [serviceScript, sourceScript] = (0, utils_1.getServiceScript)(language, fileName);
        if (serviceScript) {
            return getSyntacticDiagnostics(sourceScript.id)
                .map(d => (0, transform_1.transformDiagnostic)(language, d, false))
                .filter(d => d?.source == fileName)
                .filter(utils_1.notEmpty);
        }
    ...
    languageService.getSemanticDiagnostics = filePath => {
        ...
        const [serviceScript, sourceScript] = (0, utils_1.getServiceScript)(language, fileName);
        if (serviceScript) {
            return getSemanticDiagnostics(sourceScript.id)
                .map(d => (0, transform_1.transformDiagnostic)(language, d, false))
                .filter(d => d?.source == fileName)
                .filter(utils_1.notEmpty);
        }
    ...
    languageService.getSuggestionDiagnostics = filePath => {
        ...
        const [serviceScript, sourceScript] = (0, utils_1.getServiceScript)(language, fileName);
        if (serviceScript) {
            return getSuggestionDiagnostics(sourceScript.id)
                .map(d => (0, transform_1.transformDiagnostic)(language, d, false))
                .filter(d => d?.source == fileName)
                .filter(utils_1.notEmpty);
        }
        ...
        if (serviceScript) {
            ...
            for (const [generatedOffset, mapping] of (0, transform_1.toGeneratedOffsets)(sourceScript, map, position)) {
                ...
                for (const entry of result.entries) {
                    entry.replacementSpan = entry.replacementSpan && takeIfSameName(fileName, (0, transform_1.transformTextSpan)(sourceScript, map, entry.replacementSpan, language_core_1.isCompletionEnabled));
                }
                result.optionalReplacementSpan = result.optionalReplacementSpan
                    && takeIfSameName(fileName, (0, transform_1.transformTextSpan)(sourceScript, map, result.optionalReplacementSpan, language_core_1.isCompletionEnabled));
    ...
    languageService.provideInlayHints = (filePath, span, preferences) => {
        ...
        if (serviceScript) {
            ...
            for (const hint of result) {
                const sourcePosition = takeIfSameName(fileName, (0, transform_1.toSourceOffset)(sourceScript, map, hint.position, language_core_1.isInlayHintsEnabled));
    ...
    function sameName(name1, name2) {
        return !!name1 && !!name2 && language.normalizeId(name1) === language.normalizeId(name2);
    }
