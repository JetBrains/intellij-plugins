...
function createLanguage(plugins, caseSensitive, sync) {
    ...
    const sourceScriptToMappedFiles = new Map();
    const mappedFilesToSourceScript = new utils_1.FileMap(caseSensitive);
    ...
    function unregisterSourceScript(sourceScript) {
    ...
    function registerSourceScript(sourceScript, virtualCode) {
...
function updateVirtualCodeMapOfMap(virtualCode, mapOfMap, getSourceSnapshot) {
    ...
    for (const mapping of virtualCode.mappings) {
        ...
        if (!mapOfMap.has(source[0]) || mapOfMap.get(source[0])[0] !== source[1]) {
            mapOfMap.set(source[0], [source[1], new source_map_1.SourceMap(virtualCode.mappings, source[0])]);
        }
    }
}
