...
var require_utils2 = __commonJS({
  "node_modules/@volar/typescript/lib/node/utils.js"(exports2) {
    ...
    function fixupImpliedNodeFormatForFile(ts, pluginExtensions, sourceFile, packageJsonInfoCache, host, options) {
      ...
      const impliedNodeFormat = (_b = (_a = ts.getImpliedNodeFormatForFileWorker) == null ? void 0 : _a.call(ts, asTs, packageJsonInfoCache, host, options)) == null ? void 0 : _b.impliedNodeFormat;
...
var require_transform = __commonJS({
  "node_modules/@volar/typescript/lib/node/transform.js"(exports2) {
    ...
    function toGeneratedRange2(language, serviceScript, sourceScript, start, end, filter) {
      for (const result of toGeneratedRanges2(language, serviceScript, sourceScript, start, end, filter)) {
        return result;
      }
    }
  ...
  "node_modules/@volar/typescript/lib/node/decorateLanguageServiceHost.js"(exports2) {
    ...
    function decorateLanguageServiceHost(ts, language, languageServiceHost) {
      ...
      if (pluginExtensions.length) {
        ...
        const moduleResolutionCache = (_e = languageServiceHost.getModuleResolutionCache) == null ? void 0 : _e.call(languageServiceHost);
        if (resolveModuleNameLiterals) {
          languageServiceHost.resolveModuleNameLiterals = (moduleLiterals, containingFile, redirectedReference, options, containingSourceFile, ...rest) => {
            const disposeFixup = (0, utils_1.fixupImpliedNodeFormatForFile)(ts, pluginExtensions, containingSourceFile, moduleResolutionCache == null ? void 0 : moduleResolutionCache.getPackageJsonInfoCache(), languageServiceHost, options);
            try {
              ...
              disposeFixup == null ? void 0 : disposeFixup();
            }
          };
        }
...
var require_createLanguageServicePlugin = __commonJS({
  "node_modules/@volar/typescript/lib/quickstart/createLanguageServicePlugin.js"(exports2) {
    ...
    function createLanguageServicePlugin2(createPluginCallback) {
      return (modules) => {
...
var AngularVirtualCode = class {
    ...
    if (transpiledTemplate && this.isTemplateInSync(ts, snapshot, ctx, transpiledTemplate)) {
      ...
      transpiledTemplate.mappings.forEach((mappingSet) => {
        ...
        if (normalizedMappingFileName === ts.server.toNormalizedPath(this.fileName)) {
          ...
          const scriptId = mappingSet.externalFile ? normalizedMappingFileName : (_a = ctx.getAssociatedScript(mappingSet.fileName)) == null ? void 0 : _a.id;
          if (scriptId) {
  ...
  isTemplateInSync(ts, snapshot, ctx, transpiledTemplate) {
    ...
    if (sourceText !== transpiledTemplate.sourceCode[ts.server.toNormalizedPath(this.fileName)]) {
      ...
      var _a;
      return !mapping.externalFile && getNormalizedSnapshotText((_a = ctx.getAssociatedScript(mapping.fileName)) == null ? void 0 : _a.snapshot) !== transpiledTemplate.sourceCode[ts.server.toNormalizedPath(mapping.fileName)];
    })) {
      return false;
    }
    return true;
  }
  requiresTrailingSpaces(snapshot, transpiledTemplate) {
    const sourceText = getNormalizedSnapshotText(snapshot);
...
function createUnboundReverseMapper(language, languageService) {
  return function(ts, sourceFile, generatedRange) {
    const [serviceScript, targetScript] = (0, import_utils.getServiceScript)(language, sourceFile.fileName);
    if (targetScript == null ? void 0 : targetScript.associatedOnly) {
      return void 0;
    }
    ...
    if (serviceScript && sourceFile && generatedFile) {
      try {
        ...
        const sourceRange = toSourceRange(void 0, language, serviceScript, generatedRangePosStart, generatedRangePosEnd, true, (it) => it.types || it.reverseTypes);
        if (sourceRange !== void 0) {
          let targetFile = program == null ? void 0 : program.getSourceFile(sourceRange[0]);
          if (!targetFile)
            return void 0;
          return {
...
function decorateIdeLanguageServiceExtensions(language, languageService, unboundReverseMapper) {
  ...
  if (webStormGetElementType === void 0 || webStormGetTypeProperties === void 0 || webStormGetTypeProperty === void 0 || webStormGetSymbolType === void 0 || webStormGetCompletionSymbols === void 0)
    return;
  function withReverseMapper(source) {
    return (options) => source(__spreadProps(__spreadValues({}, options), {
  ...
  languageService.webStormGetElementType = (options) => {
    ...
    if (serviceScript && sourceFile && generatedFile) {
      ...
      if (generatedRange !== void 0) {
        return webStormGetElementType(__spreadProps(__spreadValues({}, options), {
...
function decorateNgLanguageServiceExtensions(language, languageService, unboundReverseMapper, webStormGetElementType) {
  languageService.webStormNgGetGeneratedElementType = (ts, fileName, range, forceReturnType, cancellationToken) => {
    let program = languageService.getProgram();
    if (!program) {
      return void 0;
    }
    const [_serviceScript, targetScript, _sourceScript] = (0, import_utils.getServiceScript)(language, fileName);
    const generatedFile = targetScript ? program == null ? void 0 : program.getSourceFile(targetScript.id) : void 0;
    if (!generatedFile)
      return void 0;
...
var BinarySearchStorage = class {
  constructor(originalMappings, key) {
    ...
    const offsets = [...offsetsSet].sort((a, b) => a - b);
    const mappings = offsets.map((offset) => [offset, /* @__PURE__ */ new Set()]);
    for (const mapping of originalMappings) {
      for (let i = 0; i < mapping[key].length; i++) {
...
function restoreNgTranspiledTemplateRoots(ts, project) {
  let restoredAny = false;
  ngTranspiledTemplates.forEach((transpiledTemplate, fileName) => {
    var _a;
    if (!project.containsFile(ts.server.toNormalizedPath(fileName)))
      return;
    (_a = transpiledTemplate.mappings) == null ? void 0 : _a.forEach((mapping) => {
...
function patchProjectUpdateGraphToRestoreNgRoots(ts) {
  const projectPrototype = ts.server.Project.prototype;
  if (projectPrototype.webStormNgRootsRestorePatched)
    return;
  projectPrototype.webStormNgRootsRestorePatched = true;
  const originalUpdateGraph = projectPrototype.updateGraph;
  projectPrototype.updateGraph = function(...args) {
...
function ngTranspiledTemplateHandler(ts, session, projectService, request) {
  ...
  if (scriptInfo) {
    (_a = scriptInfo.containingProjects) == null ? void 0 : _a.forEach((project) => restoreNgTranspiledTemplateRoots(ts, project));
    session.change(
      {
        file: fileName,
        line: 1,
