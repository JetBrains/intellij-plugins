...
class Angular2TagsTest : Angular2CodeInsightFixtureTestCase() {
  override fun getTestDataPath(): String {
    return Angular2TestUtil.getBaseTestDataPath() + "deprecated/tags"
  }
  ...
  fun testCustomTagsResolve20TypeScriptComponent() {
    ...
    assertEquals("""
                   ...
                   """.trimIndent(), Angular2TestUtil.getDirectiveDefinitionText(resolve))
  }
  ...
  fun testNgContainerResolve20() {
    ...
    val offsetBySignature = Angular2TestUtil.findOffsetBySignature("ng-<caret>container", myFixture.getFile())
    ...
    assertEquals("<ng-container", Angular2TestUtil.getDirectiveDefinitionText(resolve))
  }
  ...
  fun testNgTemplateResolve20() {
    ...
    val offsetBySignature = Angular2TestUtil.findOffsetBySignature("ng-<caret>template", myFixture.getFile())
    val ref = myFixture.getFile().findReferenceAt(offsetBySignature)
    assertNotNull(ref)
    val resolve = ref!!.resolve()
    assertNotNull(resolve)
    assertEquals("ngTemplate.after.html", resolve!!.getContainingFile().getName())
    assertEquals("<ng-template", Angular2TestUtil.getDirectiveDefinitionText(resolve))
  }
  ...
  fun testNgContentResolve20() {
    ...
    val offsetBySignature = Angular2TestUtil.findOffsetBySignature("ng-<caret>content", myFixture.getFile())
    val ref = myFixture.getFile().findReferenceAt(offsetBySignature)
    assertNotNull(ref)
    val resolve = ref!!.resolve()
    assertNotNull(resolve)
    assertEquals("ngContent.after.html", resolve!!.getContainingFile().getName())
    assertEquals("<ng-content", Angular2TestUtil.getDirectiveDefinitionText(resolve))
  }
