...
class Angular2InjectionsTest : Angular2CodeInsightFixtureTestCase() {
  override fun getTestDataPath(): String {
    return Angular2TestUtil.getBaseTestDataPath() + "deprecated/injections"
  }
  ...
  private fun <T : JSElement?> checkVariableResolve(signature: String, varName: String, varClass: Class<T>) {
    Angular2TestUtil.checkVariableResolve(signature, varName, varClass, myFixture)
  }

  fun testStyles2() {
    myFixture.configureByFiles("custom.ts", "package.json")
    val offset = Angular2TestUtil.findOffsetBySignature("Helvetica <caret>Neue", myFixture.getFile())
  ...
  fun testNoInjectionInHTMLTemplateLiteral() {
    ...
    val offset = Angular2TestUtil.findOffsetBySignature("b<caret>ar", myFixture.getFile())
    assert(offset > 0)
  ...
  fun testCustomContextProvider() {
    ...
    PolyContext.WEB_SYMBOLS_CONTEXT_EP
      .point!!
      .registerExtension(
        WebSymbolsContextProviderExtensionPoint(PolyContext.KIND_FRAMEWORK, "angular", object : WebSymbolsContextProvider {
